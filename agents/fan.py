...
class FANAgent(flax.struct.PyTreeNode):
    """Flow-Anchored Noise-conditioned Q-learning (FAN) agent."""
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        # Target Q.
        target_q = batch['rewards'] + self.config['discount'] * batch['masks'] * (next_z - self.config['alpha2'] * br_mse)
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            alpha2=10.0,       # Critic BC Flow Anchoring coefficient (need to be tuned for each environment).
