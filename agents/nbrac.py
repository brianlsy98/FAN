...
class NBRACAgent(flax.struct.PyTreeNode):
    """Noise-conditioned Behavior-Regularized Actor-Critic (NBRAC) agent."""
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        br_mse = jnp.square(actor_next_actions - batch['next_actions']).sum(axis=-1)
        next_z = next_z - self.config['alpha2'] * br_mse
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        ### Behavior Regularization loss.
        br_mse = jnp.square(actor_actions - batch['actions']).sum(axis=-1)
        br_loss = br_mse.mean()
        ...
        # Total loss.
        actor_loss = self.config['alpha1'] * br_loss + q_loss + z_loss
