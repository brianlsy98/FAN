...
class ValueFlowsAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss_online(self, batch, grad_params, rng):
        ...
        noises_repeated = jnp.repeat(
            jnp.expand_dims(noises, -2),
            batch['observations'].shape[-2],
            axis=-2,
        )
        observations_repeated = jnp.repeat(
            jnp.expand_dims(batch['observations'], -2),
            noises_repeated.shape[-2],
            axis=-2,
        )
        target_flow_actions = self.compute_flow_actions(noises_repeated, observations_repeated)
        actor_actions = self.network.select('actor_onestep_flow')(batch['observations'], noises, params=grad_params)
        distill_loss = jnp.mean((actor_actions - target_flow_actions) ** 2)

        # Q loss.
        actor_actions = jnp.clip(actor_actions, -1, 1)
        n1_rng, n2_rng = jax.random.split(rng, 2)
        n1_noises = jax.random.normal(n1_rng, (batch_size, 1))
        n2_noises = jax.random.normal(n2_rng, (batch_size, 1))
        qs_1 = self.network.select('critic_flow1')(n1_noises, jnp.zeros_like(n1_noises), batch['observations'], actions=actor_actions)
        qs_2 = self.network.select('critic_flow2')(n2_noises, jnp.zeros_like(n2_noises), batch['observations'], actions=actor_actions)
        qs = (qs_1 + qs_2) / 2
        ...
        # Total loss.
        actor_loss = bc_flow_loss + self.config['online_alpha'] * distill_loss + q_loss
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        if not self.config['online_switch']:
            actor_loss, actor_info = self.actor_loss(batch, grad_params, actor_rng)
            for k, v in actor_info.items():
                info[f'actor/{k}'] = v
        
        if self.config['online_switch']:
            actor_loss, actor_info = self.actor_loss_online(batch, grad_params, actor_rng)
            for k, v in actor_info.items():
                info[f'actor/{k}'] = v
    ...
    def sample_actions(
        ...
        if self.config['online_switch']:
            return self.sample_actions_online(observations, seed, temperature)
        else:
            return self.sample_actions_offline(observations, seed, temperature)

    @jax.jit
    def sample_actions_offline(
        self,
        observations,
        seed=None,
        temperature=1.0,
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            online_switch=False,  # Whether to switch to online fine-tuning.
            online_alpha=30,
