...
class Value(nn.Module):
    ...
    def __call__(self, observations, actions=None, noises=None):
...
class ShortcutModel(nn.Module):
    """Shortcut model for flow matching with stpe size conditioning.
    ...
    @nn.compact
    def __call__(self, observations, actions, times=0, step_sizes=0, is_encoded=False):
        """Return the vectors at the given states, actions, and times (optional).
        ...
        Args:
            ...
            actions: Actions.
            times: Times (optional).
            step-sizes: Step sizes for shortcut conditioning.
            is_encoded: Whether the observations are already encoded.
        """
        if not is_encoded and self.encoder is not None:
            observations = self.encoder(observations)
