...
config_flags.DEFINE_config_file('agent', 'agents/fan.py', lock_config=False)
...
def main(_):
    ...
    for dataset in [train_dataset, val_dataset, replay_buffer]:
        if dataset is not None:
            ...
            if config['agent_name'] in ['rebrac', 'nbrac']:
        ...
        if i <= FLAGS.offline_steps:
            ...
            if i == FLAGS.offline_steps + 1:
                if config['agent_name'] == 'fan' or config['agent_name'] == 'faql':
                    ...
                    new_config = flax.core.freeze(config_dict)
                    agent = agent.replace(config=new_config)
